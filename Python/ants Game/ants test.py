...
# Name:Junhao Wu, Christopher Quan    
# Email:wu.junhao@berkeley.edu, chquan@berkeley.edu
...
class Place:
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            "*** YOUR CODE HERE ***"
            assert 'Two ants in {0}'.format(self)
            if self.ant == None:
                self.ant = insect
                return
            assert not self.ant.can_contain(insect) or not insect.can_contain(self.ant)
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            "*** YOUR CODE HERE ***"
            if type(self.ant) is QueenAnt and self.ant.impostor == False:
                return
...
class Ant(Insect):
    ...
    def can_contain(self,other):
        if self.container == True and other.container == False:
            return self.ant == None
        return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        "*** YOUR CODE HERE ***"
        place = self.place
        min,max = self.min_range,self.max_range
        while min > 0 or (max>0 and place.bees==[]):
...
class QueenAnt(ScubaThrower):
    ...
    def find_ants(self):
        ...
        while front != None:
            if front.ant!= None:
                if type(front.ant) is BodyguardAnt:
                    ant_list.append(front.ant)
                    if type(front.ant.ant) is not QueenAnt:
                        ant_list.append(front.ant.ant)
        ...
        while back != None:
            if back.ant != None:
                if type(back.ant) is BodyguardAnt:
                    ant_list.append(back.ant.ant)
                    ant_list.append(back.ant)
                else:
                    ant_list.append(back.ant)
            back = back.exit
        return ant_list
...
def apply_effect(effect, bee, duration):
    ...
    "*** YOUR CODE HERE ***"
    move,status = effect(Bee.action)
    if status == 'slow' and bee.effects[status]!=0:
        bee.effects[status] += duration-1
    else :
        bee.effects[status] += duration
    def apply(colony):
        nonlocal move,status
        move(bee,colony)
        if bee.effects['stun'] > 0:
            bee.effects['stun'] -= 1
        if bee.effects['slow'] > 0:
            bee.effects['slow'] -= 1
        
        if bee.effects['slow'] == 0 and bee.effects['stun'] == 0:
            move = Bee.action
        elif bee.effects['slow']==0:
            move,status= make_stun(Bee.action)
        elif bee.effects['stun']==0:
            move,status= make_slow(Bee.action)
