...
class LambdaProcedure(Procedure):
    ...
    def apply(self, args, env):
        if proper_tail_recursion:
            ...
            val = scheme_eval(self.body, frame)
            return (val, None)
...
class MuProcedure(LambdaProcedure):
    ...
    def apply(self, args, env):
        if proper_tail_recursion:
            ...
            "*** YOUR CODE HERE ***"
            value = scheme_eval(self.body, env.make_call_frame(self.formals, args))
            return (value, None)
...
def do_lambda_form(vals, env, function_type=LambdaProcedure):
    ...
    "*** YOUR CODE HERE ***"
    if len(vals.cdr()) > 1:
        a = Pair('begin', vals.cdr())
        return (function_type(formals, a, env), env)
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_symbolp(target[0]):
            body = Pair(target.second, vals.second)
            env.define(target[0], do_lambda_form(body, env)[0])
...
def do_quote_form(vals, env):
    ...
    check_form(vals, 1, 1)
    "*** YOUR CODE HERE ***"
    return (scheme_eval(vals[0], None), None)
...
def do_let_form(vals, env):
    ...
    "*** YOUR CODE HERE ***"
    for x in bindings:
        names = Pair(x[0], names)
        values = Pair(scheme_eval(x[1], env), values)
...
def do_if_form(vals, env):
    ...
    if len(vals) == 3:
        ...
        return (scheme_eval(okay, env), None)
    ...
    for i, clause in enumerate(vals):
        ...
        if test:
            ...
            if len(clause) == 1:
                ...
                form = do_begin_form(clause.second, env)
                return scheme_eval(form, None)
    return okay, None
...
def check_formals(formals):
    ...
    for symbol in formals:
        if not scheme_symbolp(symbol):
            raise SchemeError('not a valid symbol: ' + str(symbol))
    for k in range(length):
        for y in range(k + 1 , length):
            if formals[k] == formals[y]:
                raise SchemeError('repeated symbol: ' + str(formals[k]))
