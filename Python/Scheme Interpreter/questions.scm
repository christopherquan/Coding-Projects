...
(define (list-partitions total max-pieces max-value)
  ; *** YOUR CODE HERE ***
  (define (helper total max-pieces max-value part current)
    (cond ((or (< total 0) (< max-pieces 0) (< max-value 0)) ())
      ((equal? total 0) (cons current part))
      (else (append (helper total max-pieces (- max-value 1) part current)
        (helper (- total max-value) (- max-pieces 1) max-value part (append current (list max-value)))))))
...
(define (tree-sums tree)
  ...
  (define (flatten listt)
    (cond ((null? listt) ())
      ((number? listt) (list listt))
      (else (append (flatten (car listt))
        (flatten (cdr listt))))))
  (if (null? (children tree)) (list (entry tree))
     (map (lambda (a) (+ a (entry tree))) (flatten (map tree-sums (children tree))))))
