...
def analyze_tweet_sentiment(tweet):
    ...
    for word in tweet_words(tweet):
        if has_sentiment(get_word_sentiment(word)):
            counter += 1
            average = sentiment_value(get_word_sentiment(word)) + average
    if counter == 0:
        return make_sentiment(None)    
    average  = make_sentiment(average/counter) 
    return average
...
def find_state_center(polygons):
    ...
    """
    "*** YOUR CODE HERE ***"
    cixai, ciyai, sum_ai = 0,0,0
    for poly in polygons:
        cix,ciy,ai = find_centroid(poly)
        cixai = cixai + (cix*ai)
        ciyai = ciyai + (ciy*ai)
        sum_ai = sum_ai + ai
    cx = cixai/sum_ai
...
def group_tweets_by_state(tweets):
    ...
    for key in us_states:
        ...
        state_centers[key] = center
    for tweet in tweets:
        location = tweet_location(tweet)
        closest_distance = 10000000000000000000000000
        for key in state_centers:
            distance = geo_distance(state_centers[key],location)
    ...
    for key in tweets_by_state:
        ...
        for tweet in tweets_by_state[key]:
            ...
            if has_sentiment(sent):
                value += sentiment_value(sent)
                counter += 1
        if counter != 0 and value != 0:
            averaged_state_sentiments[key] = value/counter       
