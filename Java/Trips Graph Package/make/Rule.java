...
class Rule {
    ...
    void addDependency(Rule dependent) {
        int endVertex = dependent.getVertex();
        if (!_depends.contains(endVertex)) {
            _depends.add(dependent);
        }
        _depends.add(getVertex(), endVertex);
    }
    ...
    void addCommands(List<String> commands) {
        if (!commands.isEmpty() && !_commands.isEmpty()) {
            throw new IllegalStateException("Error in adding commands.");
        } else {
    ...
    private void checkFinishedDependencies() {
        for (int i : _depends.successors(_vertex)) {
            ...
            if (r.isUnfinished()) {
                error("error: Unfinished dependency");
            }
        }
    }
    ...
    private boolean outOfDate() {
        ...
        for (int i : _depends.successors(_vertex)) {
            Rule r = _depends.getLabel(i);
            if (r.getTime() != null && getTime() < r.getTime()) {
                return true;
            }
        }
        return false;
