...
class CommandInterpreter {
    ...
    Table selectClause() {
        ...
        if (_input.nextIs(",")) {
            ...
            List<Condition> cond = conditionClause(tab1, tab2);
            Table out = tab1.select(tab2, col, cond);
            return out;
        }
        List<Condition> cond = conditionClause(tab1);
        Table out = tab1.select(col, cond);
        return out;
    }
