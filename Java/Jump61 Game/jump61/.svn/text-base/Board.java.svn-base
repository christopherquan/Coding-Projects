...
abstract class Board extends Observable {
    ...
    final Side getWinner() {
        Side current = whoseMove();
        Side opp = current.opposite();
        if (numOfSide(current) == size() * size()) {
            return current;
        }
        if (numOfSide(opp) == size() * size()) {
            return opp;
        }
        return null;
    }
    ...
    public String toString() {
        ...
        for (int i = 0; i < size() * size(); i++) {
            ...
            s = get(i);
            out += " ";
            out += Integer.toString(s.getSpots());
            if (s.getSide() == WHITE) {
                out += "-";
            } else if (s.getSide() == RED) {
                out += "r";
            } else {
                out += "b";
            }
