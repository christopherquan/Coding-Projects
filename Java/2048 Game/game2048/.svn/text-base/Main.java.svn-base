...
public class Main {
    ...
    /** End score you want to reach. */
    static final int GAME = 2048;
    ...
    boolean tiltBoard(Side side) {
                ...
                if (board[r][c] != 0) {
                    if (r != 0 && board[r - 1][c] == board[r][c]) {
                        _game.mergeTile(board[r][c], 2 * board[r][c],
                            ...
                            tiltRow(side, r - 1, c), tiltCol(side, r - 1, c));
                        board[r - 1][c] = -2 * board[r][c];
                        board[r][c] = 0;
                        _score += -1 * board[r - 1][c];
                        _count -= 1;
                        continue;
                    }
                    for (int i = 0; i < r; i++) {
                        if (board[i][c] == 0) {
                            if (i != 0 && board[i - 1][c] == board[r][c]) {
                                _game.mergeTile(board[r][c],
                                    2 * board[r][c], tiltRow(side, r, c),
                                    tiltCol(side, r, c), tiltRow(side, i - 1,
                                        c), tiltCol(side, i - 1, c));
                                board[i - 1][c] = -2 * board[r][c];
                                board[r][c] = 0;
                                _score += -1 * board[i - 1][c];
                                _count -= 1;
                            } else {
                                board[i][c] = board[r][c];
                                board[r][c] = 0;
                                _game.moveTile(board[i][c],
                                    tiltRow(side, r, c), tiltCol(side, r, c),
                                    tiltRow(side, i, c), tiltCol(side, i, c));
                            }
                            break;
                        }
                    }
        ...
        for (int r = 0; r < SIZE; r += 1) {
            for (int c = 0; c < SIZE; c += 1) {
                if (_board[tiltRow(side, r, c)][tiltCol(side, r, c)]
                    != Math.abs(board[r][c]) && !changed) {
                    changed = true;
                }
                _board[tiltRow(side, r, c)][tiltCol(side, r, c)] =
                    Math.abs(board[r][c]);
